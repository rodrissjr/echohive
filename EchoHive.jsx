...
const Feed = ({ posts, activeCategory, onCategoryChange, onReact, onOpen, onDelete, onBookmark, onShare, onRepost, isAdmin, currentUserId, onCreate, title, subtitle, emptyState }) => {
...
const PostDetail = ({ post, comments, onClose, onReact, onAddComment, onBookmark, onShare, onRepost, currentUser }) => {
  ...
  const submit = () => {
    if (!text.trim()) return;
    onAddComment(post.id, text.trim(), null);
    setText('');
  };
          ...
          <div className="flex gap-3 mb-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex justify-end mt-2">
                <button onClick={submit} className="eh-btn eh-btn-primary text-sm" style={{ padding: '7px 16px' }} disabled={!text.trim()}>
                  Reply
                </button>
              </div>
            </div>
        ...
        <div style={{ padding: 24 }}>
          ...
          <div className="flex justify-between items-center mt-6">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => canSubmit && onSubmit({ title: title.trim(), content: content.trim(), category })}
                disabled={!canSubmit} className="eh-btn eh-btn-primary text-sm"
                style={{ padding: '8px 20px', opacity: canSubmit ? 1 : 0.4, cursor: canSubmit ? 'pointer' : 'not-allowed' }}>
                Publish
              </button>
            </div>
          </div>
...
const AuthView = ({ onLogin, toast }) => {
  ...
  const handleLogin = (e) => {
    ...
    onLogin({
      id: 1,
      username: form.username || 'james.k',
      display: form.display || 'James Kalolo',
      email: form.email || 'james@iaa.ac.tz',
      university: form.university || 'Institute of Accountancy Arusha',
      role: form.email === 'admin@iaa.ac.tz' ? 'admin' : 'student',
    });
  };
  const handleRegister = (e) => {
    ...
    onLogin({
      id: Date.now(),
      username: form.username || 'new.user',
      display: form.display || 'New User',
      email: form.email,
  ...
  const handleForgot = (e) => {
    e.preventDefault();
    toast(`Reset link sent to ${form.email || 'your email'}`);
        ...
        <div className="eh-card eh-fade-up" style={{ padding: '36px 36px 32px', animationDelay: '120ms' }}>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  ...
                  <button type="button" onClick={() => setMode('forgot')} className="font-display"
              ...
              <button type="submit" className="eh-btn eh-btn-primary w-full" style={{ padding: '11px', fontSize: 14, fontWeight: 600 }}>
                Sign in
              </button>
              <div className="font-body text-center" style={{ fontSize: 13, color: 'var(--ink-2)' }}>
                New to EchoHive?{' '}
                <button type="button" onClick={() => setMode('register')} className="font-display" style={{ color: 'var(--accent-deep)', fontWeight: 600 }}>
                  Create an account
                </button>
              </div>
          ...
          {mode === 'register' && (
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div className="font-body text-center" style={{ fontSize: 13, color: 'var(--ink-2)' }}>
                Already on EchoHive?{' '}
                <button type="button" onClick={() => setMode('login')} className="font-display" style={{ color: 'var(--accent-deep)', fontWeight: 600 }}>
                  Sign in instead
                </button>
              </div>
            </form>
...
export default function EchoHive() {
  ...
  const handleReact = (postId, type) => {
    setPosts(prev => prev.map(p => {
      ...
      const prev_ = p.userReaction;
      if (prev_ === type) {
        reactions[type] = Math.max(0, reactions[type] - 1);
        return { ...p, reactions, userReaction: null };
      }
      if (prev_) reactions[prev_] = Math.max(0, reactions[prev_] - 1);
      reactions[type] = (reactions[type] || 0) + 1;
      return { ...p, reactions, userReaction: type };
    }));
  };
  ...
  const handleCreate = ({ title, content, category }) => {
    const newPost = {
      ...
      author: { id: user.id, username: user.username, display: user.display, university: user.university, year: 'Year 4 • BCSe' },
      createdAt: Date.now(),
      reactions: { like: 0, love: 0, laugh: 0, wow: 0, sad: 0, angry: 0 },
      userReaction: null, comments: 0, views: 0, reposts: 0,
      isBookmarked: false, isReposted: false,
    };
  ...
  const handleAddComment = (postId, content, parentId = null) => {
    const newComment = {
      id: Date.now(), postId, parentId,
      author: { username: user.username, display: user.display },
  ...
  const handleRepost = (postId) => {
    ...
    if (!post) return;
    if (post.isReposted) {
      setPosts(prev => prev.map(p => p.id === postId ? { ...p, isReposted: false, reposts: Math.max(0, p.reposts - 1) } : p));
  ...
  const confirmRepost = (comment) => {
    setPosts(prev => prev.map(p => p.id === repostTarget.id ? { ...p, isReposted: true, reposts: p.reposts + 1 } : p));
  ...
  const handleBan   = (uid) => { setUsers(prev => prev.map(u => u.id === uid ? { ...u, isBanned: true } : u)); showToast('User banned'); };
  const handleUnban = (uid) => { setUsers(prev => prev.map(u => u.id === uid ? { ...u, isBanned: false } : u)); showToast('User restored'); };
